...
3. If using webhook mode, set `IMESSAGE_API_KEY` (see [Webhook](#webhook) below)
...
- `apiKey` must be configured on the adapter (via `IMESSAGE_API_KEY` env var or the `apiKey` config option)
- The `imessage-kit` instance must send the `x-imessage-gateway-token` header with the same API key value
...
| `apiKey` | Remote mode; webhook mode | API key for authentication. Also used to verify `x-imessage-gateway-token` in webhook mode. Auto-detected from `IMESSAGE_API_KEY` |
...
IMESSAGE_API_KEY=...                 # Required for remote mode and webhook mode
```
